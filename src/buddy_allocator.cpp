...
BuddyAllocator::BuddyAllocator(uint64_t size, uint64_t min_buddy_size, Allocator &main_allocator,
    ...
    // Now, there's could be a portion to the left of the buffer which we must
    // make unavailable by marking it as allocated. This is the size of that
    // portion. The rest of this code deals with that.
    _unavailable = clip_to_power_of_2(_buffer_size - size);
    _mem = _mem - _unavailable;

    // If size was a power of of two, all buddies are free, otherwise, we have
    // to artifically mark the dummy buddy of size `_unavailable` at toward
    // the left
    if (_unavailable == 0) {
    ...
    debug("BuddyAllocator::Initialized\n"
          ...
          "_unavailable = %lu\n"
          "_num_indices = %lu\n------\n",
          _buffer_size, _leaf_buddy_size, _leaf_buddy_size_power, _num_levels, _buffer_size - _unavailable,
          _unavailable, 1lu << _last_level());
}
...
void *BuddyAllocator::allocate(uint64_t size, uint64_t align) {
    ...
    log_assert(size >= _leaf_buddy_size, "Cannot allocate a buddy size smaller than the minimum size of %lu",
               _leaf_buddy_size);
...
void BuddyAllocator::deallocate(void *p) {
    ...
    if (!((int64_t)_total_allocated - (int64_t)size >= 0)) {
        log_err("BuddyAlloc::Unallocated index (index_allocated = %d?) "
                ...
                int(_leaf_allocated.get(idx)), idx, level, size, _total_allocated);
        abort();
    }
    ...
    while (level >= 1) {
        ...
        // _leaves_contained(n) buddies of size _buddy_size_at_level(n). We
...
BuddyHead *BuddyAllocator::_break_free(uint64_t level) {
    ...
    if (!(_level_of_leaf.get(_leaf_index(h1)) == level)) {
        log_err("h1's level(index = %lu) = %lu, but found in level %lu", _leaf_index(h1),
                _level_of_leaf.get(_leaf_index(h1)), level);
        abort();
    }
    if (!(_level_of_leaf.get(_leaf_index(h2)) == level)) {
        log_err("h2's(index = %lu) level = %lu, but found in level %lu", _leaf_index(h2),
                _level_of_leaf.get(_leaf_index(h2)), level);
        abort();
    }
#endif
...
void BuddyAllocator::_check_leaf_index(BuddyHead *p) const {
    ...
    if (mod != 0) {
        log_err("Buddy block of size %lu can never begin at index %lu and be at level %lu",
                buddies_inside << _leaf_buddy_size_power, index, level);
        abort();
    }
#else
    (void)p;
