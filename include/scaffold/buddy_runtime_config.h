...
private:
  ...
  struct BuddyHead {
    ...
    BuddyHead(BuddyHead *next, BuddyHead *prev) : _next(next), _prev(prev) {}
...
private:
  ...
  static inline constexpr uint64_t _buddy_size_at_level(uint32_t level) {
    return _buffer_size >> (uint64_t)level;
  }

private:
  uint64_t _buffer_size;
  uint32_t _min_buddy_size;
  /// Number of levels max
  uint32_t _num_levels = log2_ceil(_buffer_size / _min_buddy_size) + 1;
  /// Last level (for convenience)
  uint32_t _last_level = _num_levels - 1;
  /// Minimum buddy size power
  uint32_t _min_buddy_size_power = log2_ceil(_min_buddy_size);
  /// Number of buddy indices
  uint32_t _num_indices;

public:
  /// Alignment
  uint32_t _buddy_alignment = _min_buddy_size;
...
public:
  BuddyAllocator(Allocator &backing_allocator, uint64_t buffer_size,
                 uint32_t min_buddy_size)
      : _backing(&backing_allocator), _mem(nullptr) {
    _initialize_parameters(buffer_size, min_buddy_size);
    _initialize();

    debug("BuddyAllocator::Initialized - _buffer_size = %lu "
          "_min_buddy_size = %u, _min_buddy_size_power = %u, "
          "_num_levels = %u, num_indices = %u\n--",
          _buffer_size, _min_buddy_size, _min_buddy_size_power, _num_levels,
          1 << _last_level);
  }
  ...
  void *allocate(uint32_t size, uint32_t align) override {
    ...
    while (true) {
      ...
      if (buddy_size > size) {
        ...
        debug("BuddyAlloc::Allocate::Break i:%u - level - %d", index, level);
        _break_free(level);
        print_level_map(index, index + _buddies_contained(level));
        ++level;
        print_level_map();
        ...
        debug("BuddyAlloc::Found exact size(%lu buddies) TO ALLOC - "
              "i:%lu",
              next_index, index);
        print_level_map(index,
                        next_index >= _num_indices ? _num_indices : next_index);
        h->remove_self_from_list(_free_lists, level);
        // Set each to allocated status
        for (uint32_t b = index, e = index + _buddies_contained(level); b < e;
             ++b) {
  ...
  void deallocate(void *p) override {
    ...
    debug("BuddyAlloc::deallocate - set initial block headed by i:%u to free",
          idx);
    print_level_map(idx, idx + 20 >= _num_indices ? _num_indices : idx + 20);

    while (level >= 1) {
      debug("BuddyAlloc::MERGE START - %d, level - %d", original_level - level,
            level);
      print_level_map();
      ...
      if (!_index_allocated.get(left_idx) && !_index_allocated.get(right_idx) &&
        ...
        debug("BuddyAlloc::AFTER MERGE - left i:%u and right i:%u", left_idx,
              right_idx);
        print_level_map(idx,
                        idx + 20 >= _num_indices ? _num_indices : idx + 20);
  ...
  void print_level_map(uint32_t start = 0, uint32_t end = _num_indices) const {
...
private:
  void _initialize_parameters(uint64_t buffer_size, uint32_t min_buddy_size) {
    _buffer_size = buffer_size;
    _min_buddy_size = min_buddy_size;
    _num_levels = log2_ceil(_buffer_size / _min_buddy_size) + 1;
    _last_level = _num_levels - 1;
    _min_buddy_size_power = log2_ceil(_min_buddy_size);
    _num_indices = 1 << _last_level;
    ...
    assert((_buffer_size <= uint64_t(4) << 30) &&
           "Buffer size should not exceed 4-GB");
    assert(_num_levels > 0 && "_num_levels must be > 0");
    assert(_min_buddy_size >= sizeof(BuddyHead) &&
           "minimum buddy size is not sufficient with this config");
    assert((_min_buddy_size & (_min_buddy_size - 1)) == 0 &&
           "_min_buddy_size should be a power of 2");
    assert(clip_to_power_of_2(_buffer_size) == _buffer_size &&
           "buffer size must be a power of 2");
  }

  /// Allocates the buffer and sets up the free lists.
  void _initialize() {
    _mem = _backing->allocate(_buffer_size, _buddy_alignment);
  ...
  BuddyHead *_break_free(uint32_t level) {
    ...
    if (!(_level_of_index.get(_buddy_index(h1)) == level)) {
      log_err("h1's level(index = %u) = %u, but found in level %u",
              _buddy_index(h1), _level_of_index.get(_buddy_index(h1)), level);
      assert(0 && "See error");
    }
    if (!(_level_of_index.get(_buddy_index(h2)) == level)) {
      log_err("h2's(index = %u) level = %u, but found in level %u",
              _buddy_index(h2), _level_of_index.get(_buddy_index(h2)), level);
      assert(0 && "See error");
    }
#endif
