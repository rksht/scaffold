...
//
// The metadata is kept separately (for now) and not in the buffer managed by
// the allocator.

namespace buddy_allocator_internal {
/// Forward declaring the type for headers used to chain the free buddies of
...
class BuddyAllocator : public Allocator {
  private:
    ...
    DySmallIntArray<uint64_t> _leaf_allocated;         // Is buddy with the given index, allocated?
    DySmallIntArray<uint64_t> _level_of_leaf;          // At which level is the buddy residing?
  ...
  private:
    ...
    /// Returns the number of min-buddies contained in each buddy of given
    /// `level`
    uint64_t _leaves_contained(uint64_t level) const;

    /// Called by the constructor when the total size to manage is not a power
    /// of 2
