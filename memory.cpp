...
namespace {
	...
	/// A ArenaAllocator allocates a buffer and bumps a pointer on each allocation.
	/// Extra information is kept in the allocated chunks and the objects can be "popped"
	...
	public:
		...
		{
			_Header* new_top_header;
			char *after_header, *aligned_start;
			uint32_t pad_size;

			assert((size % alignof(_Header) == 0) &&
                    (align % alignof(_Header) == 0));

			new_top_header = (_Header*) ((char*)_top_header + sizeof(_Header)
										+ _top_header->size);

			after_header = (char*) (new_top_header + 1);
			...
			new_top_header->size = size;
			_total_allocated += new_top_header->size + pad_size;
			_top_header = new_top_header;
	...
	struct MemoryGlobals {
		...
		MemoryGlobals() : default_allocator(0), default_scratch_allocator(0), default_arena_allocator(0) {}
	};
...
namespace foundation
{
	namespace memory_globals
	{
		void init(uint32_t temporary_memory) {
			...
			_memory_globals.default_arena_allocator = new (p) ArenaAllocator(*_memory_globals.default_allocator, 4 * 1024 * 1024);
		}
